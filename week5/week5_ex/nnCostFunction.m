...
% size of h: 5000*10    
% a1 = X
% a2 = g(z2) = g((X+x0)*Theta1')
% h = a3 = g(z3) = g((a2+x0)*Theta2')
...
J = J + (sum(sum(Theta1_withoutBias.*Theta1_withoutBias))+sum(sum(Theta2_withoutBias.*Theta2_withoutBias)))*lambda/(2*m);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% z2 = X *Theta1';      %5000*25
% delta3 = h - y_vec;   %5000*10
%                       %Theta2 = 10*(25+1)
% delta2 = (delta3 * Theta2) .* [ones(size(z2,1), 1) sigmoidGradient(z2)];   %5000*26
% delta2 = delta2(:,2:end); %5000*25?

% % X = 5000*400
% Theta1_grad = (1/m) * (delta2'* X); 25*400
% Theta2_grad = (1/m) * (delta3' * sigmoid(z2)); 10*25
...
% delta2 = delta2(:, 2:end);
% delta2 = delta2 .*sigmoidGradient(X_bias * Theta1');

% delta_1 = zeros(size(Theta1));
% delta_2 = zeros(size(Theta2));

% delta_1 = delta_1 + delta2'* X_bias;
